defmodule CursorDemoWeb.UserFollowingLive do
  ...
  def mount(%{"username" => username}, _session, socket) do
    ...
    case Accounts.get_user_by_username(username) do
      %User{} = user ->
        # First get the IDs of users this person follows
        following_ids = from(f in CursorDemo.Accounts.Follow,
          where: f.follower_id == ^user.id,
          select: f.followed_id
        ) |> Repo.all()

        # Then get the actual user records
        following = from(u in User,
          where: u.id in ^following_ids
        )
        |> Repo.all()
        |> Enum.map(fn followed_user ->
          Map.put(followed_user, :following?, Accounts.following?(socket.assigns.current_user, followed_user))
        end)

        socket = socket
          |> assign(
            page_title: "@#{user.username} following",
          ...
          <div class="px-4 sm:px-6 lg:px-8">
            <h1 class="text-xl font-bold text-gray-900 my-4">
              <%= if @profile_user.id == @current_user.id do %>
                People you follow
              <% else %>
                People followed by <%= @profile_user.username %>
              <% end %>
            </h1>
  ...
  def handle_event("toggle_follow", %{"user-id" => user_id}, socket) do
    ...
    user = Enum.find(socket.assigns.following, & &1.id == user_id)
    ...
    case result do
      {:ok, _} ->
        ...
        following = Enum.map(socket.assigns.following, fn u ->
          if u.id == user_id do
  ...
  def handle_info({:follow_updated, %{follower_id: follower_id, followed_id: followed_id}}, socket) do
    ...
    following = Enum.map(socket.assigns.following, fn user ->
